...
[[file:diagram.png][Deployment Diagram of Drupal into Kubernetes]]
   ...
   graph TD
      ...
      DrupalDeployment -->|uses| PVCDrupal[Persistent Volume Claim<br> for Drupal files & data ]
...
*** Deployment Diagram in plantuml
   ...
   RECTANGLE "Drupal Deployment\n(Pods with Drupal\nand PHP runtime)" as DrupalDeployment
   RECTANGLE "MariaDB Deployment\n(Pods with MariaDB)" as MariaDBDeployment
   RECTANGLE "Persistent Volume Claim\n(for Drupal files & data)" as PVCDrupal
   RECTANGLE "Persistent Volume Claim\n(for MariaDB data)" as PVCMariaDB
   RECTANGLE "Persistent Volume (PV)" as PV
   RECTANGLE "Kubernetes Secrets &\nConfigMaps" as SecretsConfigMaps

   Ingress -down-> LoadBalancer : exposes
   LoadBalancer -down-> DrupalDeployment : routes to
   DrupalDeployment --> PVCDrupal : uses
   PVCDrupal -down-> PV : bound to
   DrupalDeployment -right-> SecretsConfigMaps : reads from

   MariaDBDeployment -down-> PVCMariaDB : uses
   PVCMariaDB -up-> PV : bound to
   MariaDBDeployment -up-> SecretsConfigMaps : reads from
