    ...
    mariadb:
      resources:
        ...
        limits:
          ...
          memory: 512Mi
    #+end_src
2.  Deploy Drupal and MariaDB. This will use https://github.com/bitnami/charts/tree/main/bitnami/drupal
#+BEGIN_SRC bash
...
LoadBalancer -->|routes to| DrupalDeployment[Drupal Deployment Pods with Drupaland PHP runtime ]
...
- OPTION 1: Create and use a ckuster pilong
#+begin_src bash
...
#+end_src
- OPTION 3: If that didn't work, alternately you can forward port 8080 to the Drupal pod.
#+BEGIN_SRC bash
...
:END:
1.  Deploy Metrics Server to gather CPU and memory usage:
    #+begin_src bash
...
- For Apache:
#+BEGIN_SRC bash
kubectl exec -- pgrep httpd
#+END_SRC
- For Nginx:
#+BEGIN_SRC bash
kubectl exec -- pgrep nginx
#+END_SRC
...
- NodePort Service:
#+BEGIN_SRC bash
kubectl expose deployment --type=NodePort --name=drupal-service --port=80
#+END_SRC
- LoadBalancer Service:
#+BEGIN_SRC bash
kubectl expose deployment --type=LoadBalancer --name=drupal-loadbalancer --port=80
#+END_SRC
* Cleanup
